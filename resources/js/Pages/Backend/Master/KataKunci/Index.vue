...
<template>
 ...
 <div>
 <h3 class="text-xl font-bold text-gray-900 dark:text-white">Master Kata Kunci</h3>
 <p class="text-xs text-gray-500 mt-1">Kelola tag kata kunci pencarian kajian ilmiah daerah.</p>
 </div>
 <button 
 ...
 <tr v-for="item in keywords.data" :key="item.id" class="hover:bg-gray-50/50 dark:hover:bg-gray-800 transition-colors">
 <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">
 <span class="inline-block bg-paper-2 dark:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs font-medium px-2.5 py-1 rounded-sm border border-slate-100 dark:border-slate-600">
 #{{ item.nama }}
 </span>
 </td>
 ...
 Menampilkan {{ keywords.from || 0 }} sampai {{ keywords.to || 0 }} dari {{ keywords.total }} data
 </p>
 ...
 {{ isEdit ? 'Edit Kata Kunci' : 'Tambah Kata Kunci' }}
 </h4>
 ...
 <div>
 <label class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Nama Tag <span class="text-red-500">*</span></label>
 <input 
 type="text" 
 v-model="form.nama"
 required
