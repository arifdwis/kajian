...
class SSOAutoLogin
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($request->segment(1) == 'login' && auth()->guest()) {
            try {
                ...
                // If logged in SSO server and guest in broker, auto authenticate
                if (isset($response['data']) && auth()->guest()) {
                    $this->handleLogin($response);
                }
            } catch (\Exception $e) {
                // Log and bypass to allow normal email login
    ...
    public function handleLogin($response)
    {
        ...
        if ($check) {
            ...
            if ($check->roles()->count() === 0) {
                $check->assignRole('pengguna');
            }
            Auth::login($check);
        } else {
